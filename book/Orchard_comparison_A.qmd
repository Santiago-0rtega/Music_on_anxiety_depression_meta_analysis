...
left_col \<- (p_RR_all / p_RR_mod) + plot_layout(heights = c(1, 6)) right_col \<- (p_VR_all / p_VR_mod) + plot_layout(heights = c(1, 6))
...
comparison\<-final_patch + plot_annotation( title = '', tag_levels = 'A' ) comparison

```{r}
fig2<-(p_RR_all/p_VR_all)
