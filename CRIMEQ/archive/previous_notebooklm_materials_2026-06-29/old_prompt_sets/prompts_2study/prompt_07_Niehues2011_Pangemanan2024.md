# Prompt 07: Niehues2011 and Pangemanan2024

```text
Create a Google Sheet or Studio Data Table named "CRIME-Q music extraction part 07".
...
Before creating the table, verify that you can read both PDFs by finding each exact article title. If either PDF is unavailable, stop and report only: Source access failed for: [filename].

The table must have exactly 2 rows, in this order:
Niehues2011
Pangemanan2024
...
VERBATIM must be an exact quote from the target paper with quoted words plus a location tag, or exactly NOT REPORTED IN PAPER. A page/table/figure location alone is invalid.

Do not write "full text not available in source context" in any CRIME-Q cell. Do not put location-only citations in VERBATIM cells. Do not create columns beginning with ITEM_, columns ending in _SCORE, or columns that remove parentheses from item IDs.
